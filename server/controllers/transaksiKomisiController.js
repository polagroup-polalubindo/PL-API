...
class Controller {
  static createTransaksiKomisi = async (req, res) => {
    try {
        ...
        const transaksi = await TransaksiKomisi.create({komisiId,userId,nominal})
        const {dataValues} = await Komisi.findOne({where:{id:komisiId},include:TransaksiKomisi})
        if (dataValues.TransaksiKomisis.length === 0){
            dataValues.sisaKomisi = dataValues.totalKomisi - nominal
        }else{
            dataValues.sisaKomisi-= nominal
        }
        const updateKomisi = await Komisi.update(dataValues,{where:{id:komisiId}})
